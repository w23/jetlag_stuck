...
static __forceinline void introInit() {
	...
	*/
	...
	if (!pixels)
		abort();
	initTexture(texture[Tex_Text], w, h, n, GL_UNSIGNED_BYTE, pixels);
	...
	*/
	...
	//FILE *f = fopen("../shader.frag"
	program[Pass_2Dfx1] = compileProgram(shader_glsl);
	oglUseProgram(program[Pass_2Dfx1]);
	GLCHECK();
	//program[Pass_Blit] = compileProgram(blitter_glsl);
	int uni = oglGetUniformLocation(program[Pass_2Dfx1], "Tex");
	if (uni < 1) abort();
	oglUniform1i(uni, 0);
	GLCHECK();
...
static __forceinline void introPaint() {
	const float t = (float)itime / ((float)SAMPLES_PER_TICK * sizeof(SAMPLE_TYPE) * 2);
